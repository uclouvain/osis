...
def pdf_content(request, year, code, language):
    program_tree_version = ProgramTreeVersionRepository.get(
        ProgramTreeVersionIdentitySearch().get_from_node_identity(NodeIdentity(code=code, year=year))
    )
    ...
    if program_tree_version:
        ...
        tree = ProgramTreeRepository.get(
            ProgramTreeIdentitySearch().get_from_node_identity(NodeIdentity(code=code, year=year))
        )
