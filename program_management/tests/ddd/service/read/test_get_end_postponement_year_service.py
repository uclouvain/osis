...
class TestNodeIdentityService(MockPatcherMixin, TestCase):
    def setUp(self):
        ...
        self.program_tree = ProgramTreeFactory(
            ...
            # root_node__end_year=self.current_year,
        )
    ...
    # def test_when_root_node_is_minitraining_and_end_year_lt_maximum_postponement(self, mock_data_from_db):
    #     end_year = self.maximum_postponement_year - 3
    #     mini_training = MiniTrainingFactory(entity_identity__year=self.current_year, end_year=self.current_year)
    #     mock_data_from_db.return_value = {"offer_acronym": mini_training.entity_identity.acronym, "offer_type": mini_training.type.name}
    #     fake_minitraining_repository = get_fake_mini_training_repository([mini_training])
    ...
    #         mini_training_repository=fake_minitraining_repository
    #     )
