...
class TestDeleteOrphanMiniTrainig(TestCase, MockPatcherMixin):
    ...
    def test_should_remove_mini_training_from_repository(self):
        entity_identity_of_deleted_group = delete_orphan_mini_training_service.delete_orphan_mini_training(self.cmd)
        with self.assertRaises(exception.MiniTrainingNotFoundException):
            self.fake_mini_training_repo.get(entity_identity_of_deleted_group)
